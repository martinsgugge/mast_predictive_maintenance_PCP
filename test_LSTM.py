...
def get_data_pre_process(start, end, pump_no_tag, checkpoint_path, table, file, avg=False, std=False,
    ...
    for timestamp, measurement in zip(pump_nos.timestamp, pump_nos.measurements):
        ...
        if not skip:
            ...
            if not meta.failed:
                print(f'Getting data: meta : {meta.failed}')
                if use_csv_file:
                    try:
    ...
    return train_x, train_y, validate_x, validate_y, test_x, test_y, meta
...
def test_LSTM(date_start, date_stop, pump_state_list, checkpoint_path, table, file, use_csv_file=False):
    ...
    model = tf.keras.models.load_model(f'./LSTM_models/{meta.model_name}/{meta.previous_checkpoint_path[14:]}')
    print(f'./LSTM_models/{meta.model_name}/{meta.previous_checkpoint_path[14:]}')
...
if __name__ == '__main__':
    ...
    settings_file = [
                    #'LSTM model_settings exp 1.xml', 'LSTM model_settings exp 2.xml',
        'LSTM model_settings exp 18.xml' #Need debug
                     #'LSTM model_settings exp 4.xml', 'LSTM model_settings exp 5.xml', 'LSTM model_settings exp 6.xml',
                     #'LSTM model_settings exp 7.xml', 'LSTM model_settings exp 8.xml', 'LSTM model_settings exp 9.xml',
        # 'LSTM model_settings exp 10.xml',
        # 'LSTM model_settings exp 11.xml',
        # 'LSTM model_settings exp 12.xml', 'LSTM model_settings exp 13.xml', 'LSTM model_settings exp 14.xml',
        # 'LSTM model_settings exp 15.xml'
                     ]
    for file in settings_file:
        print(file)

        test_LSTM(date_start, date_stop, pump_state_list, checkpoint_path, table, file, use_csv_file=True)
