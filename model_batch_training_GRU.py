...
def main_training_pipeline(start, end, pump_no_tag, checkpoint_path, table, settings_file, avg=False, std=False,
    ...
    try:
        skip_type_1_pump = bool(int(computer_specifics.SearchForTag('skip_pump_type_1')))
        skip_type_2_pump = bool(int(computer_specifics.SearchForTag('skip_pump_type_2')))
    ...
    for timestamp, measurement in zip(pump_nos.timestamp, pump_nos.measurements):
        ...
        if not skip:
            ...
            if not meta.failed:
                ...
                if not exists(
                        f'./line_plots/new font_Lineplot {table} {tag} {str(timestamp)[:10]} raw={meta.raw} avg={meta.average} '
                        f'std={meta.standard_deviation} agg_time={meta.aggregation_time}.html'):
                    plot(
                        f'./line_plots/font_Lineplot {table} {tag} {str(timestamp)[:10]} raw={meta.raw} avg={meta.average} '
            ...
            if not meta.failed:
                ...
                if not testing:
                    ...
                    model = tf.keras.models.load_model(f'./GRU_models/{meta.model_name}/'
...
if __name__ == '__main__':
    ...
    settings_file = [#'GRU model_settings exp 4.xml',
                     #'GRU model_settings exp 5.xml', 'GRU model_settings exp 6.xml'
