...
def create_model(meta: Metadata):
    ...
    model.add(keras.layers.LSTM(meta.neurons_layer_one,
                                batch_input_shape=(meta.batch_size, meta.sequence_steps, meta.no_of_features),
                                return_sequences=True, stateful=meta.stateful))
    if meta.layers == 2:
        model.add(keras.layers.LSTM(meta.neurons_layer_two, return_sequences=False, stateful=meta.stateful))
    elif meta.layers == 3:
        model.add(keras.layers.LSTM(meta.neurons_layer_two, return_sequences=False, stateful=meta.stateful))
        model.add(keras.layers.LSTM(meta.neurons_layer_three, return_sequences=True, stateful=meta.stateful))
...
def train_model(model, X_train, X_test, y_train, y_test, meta: Metadata):
    ...
    if os.path.exists(f'./LSTM_models/{meta.model_name}{meta.checkpoint_path[1:]}'):
        cp_callback = keras.callbacks.ModelCheckpoint(
            filepath=f'./LSTM_models/{meta.model_name}{meta.checkpoint_path[1:]}/cp.ckpt',
            save_weights_only=True, verbose=1)
    else:
        os.makedirs(f'./LSTM_models/{meta.model_name}{meta.checkpoint_path[1:]}')
        cp_callback = keras.callbacks.ModelCheckpoint(
            filepath=f'./LSTM_models/{meta.model_name}{meta.checkpoint_path[1:]}/cp.ckpt',
            save_weights_only=True, verbose=1)
    ...
    if os.path.exists(f'./LSTM_models/{meta.model_name}'):
        model.save(f'./LSTM_models/{meta.model_name}/{meta.checkpoint_path[len("./checkpoints/"):]}')
    else:
        os.makedirs(f'./LSTM_models/{meta.model_name}')
        model.save(f'./LSTM_models/{meta.model_name}/{meta.checkpoint_path[len("./checkpoints/"):]}')
...
def define_and_train_model(X_train, X_test, y_train, y_test, meta: Metadata):
    ...
    print(meta.train_run)
    if meta.train_run is None:
        model = create_model(meta)
    else:
        model = create_model(meta)
        try:
            model.load_weights(f'./LSTM_models/{meta.model_name}{meta.previous_checkpoint_path[1:]}/cp.ckpt')
        except FileNotFoundError:
...
def test_model(model, X_test, y_test, meta: Metadata):
    ...
    try:
        ...
        # Plot confusion matrix in decimal and number of hits
        if os.path.exists(f'./cfsmatplots/LSTM/{meta.model_name}'):
            plot_confusion_matrix(lstm_cm_norm, ['Normal', 'One week from fail', 'One day from fail'],
                                  f'./cfsmatplots/LSTM/{meta.model_name}/in percent all pumps {meta.checkpoint_path[len("./checkpoints/"):]}')
        else:
            os.makedirs(f'./cfsmatplots/LSTM/{meta.model_name}')
            plot_confusion_matrix(lstm_cm_norm, ['Normal', 'One week from fail', 'One day from fail'],
                                  f'./cfsmatplots/LSTM/{meta.model_name}/in percent all pumps {meta.checkpoint_path[len("./checkpoints/"):]}')
        if os.path.exists(f'./cfsmatplots/LSTM/{meta.model_name}'):
            plot_confusion_matrix(lstm_cm, ['Normal', 'One week from fail', 'One day from fail'],
                                  f'./cfsmatplots/LSTM/{meta.model_name}/all pumps {meta.checkpoint_path[len("./checkpoints/"):]}')
        else:
            os.makedirs(f'./cfsmatplots/LSTM/{meta.model_name}')
            plot_confusion_matrix(lstm_cm, ['Normal', 'One week from fail', 'One day from fail'],
                                  f'./cfsmatplots/LSTM/{meta.model_name}/all pumps {meta.checkpoint_path[len("./checkpoints/"):]}')
