...
def get_data_pre_process(start, end, pump_no_tag, checkpoint_path, table, file, avg=False, std=False,
                         skip_type_1_pump=False, skip_type_2_pump=False):
    ...
    #meta.method = 'HalvingGridSearchCV rbf'
...
def main_training_pipeline(start, end, pump_no_tag, checkpoint_path, table, file):
    train_x = []
    train_y = []
    validate_x = []
    validate_y = []
    ...
    for tag in pump_state_list:
        X_train, Y_train, X_validate, Y_validate, X_test, Y_test, meta = \
            get_data_pre_process(start, end, tag, checkpoint_path, table, file)
...
if __name__ == '__main__':
    ...
    settings_file = ['SVM model_settings exp 19.xml'#, 'SVM model_settings exp 4.xml'
                     ]
    for file in settings_file:
        print(file)

        main_training_pipeline(date_start, date_stop, pump_state_list, checkpoint_path, table, file)
